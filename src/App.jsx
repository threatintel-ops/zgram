...
import {
  ...
  PlusSquare, Film, PlayCircle, User, X, Smile, LogOut, Camera, Volume2, VolumeX,
  ...
  HelpCircle, BadgeCheck, AtSign, MessageSquare, Share2, Check,
} from "lucide-react";
...
function compressImageToBlob(file, maxDim = 1080, quality = 0.8) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onerror = () => reject(new Error("read failed"));
    reader.onload = () => {
...
// Uploads a Blob/File into the shared "media" storage bucket under the
// current user's own folder (auth.uid()) — matches the bucket's RLS policy,
// which only allows writes into a folder named after your own user id.
// Returns the public URL to store in a posts/reels row.
async function uploadToMediaBucket(blob, extension) {
  const { data: sessionData } = await supabase.auth.getSession();
  const uid = sessionData.session.user.id;
  const path = `${uid}/${Date.now()}-${Math.random().toString(36).slice(2)}.${extension}`;
...
const DEFAULT_AVATAR =
  ...
  encodeURIComponent(
    `<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100'><rect width='100' height='100' fill='#1c1917'/><text x='50' y='58' font-size='40' text-anchor='middle' fill='#f97316' font-family='sans-serif'>?</text></svg>`
  );
...
function AuthScreen({ onAuthed }) {
  ...
  return (
    <div className="auth-backdrop min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="bg-[rgb(var(--fg-rgb)/4%)] backdrop-blur-xl border border-[rgb(var(--fg-rgb)/10%)] rounded-3xl p-8">
          <h1 className="font-brand text-center text-6xl tracking-tight mb-1 bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent">
            Z
          </h1>
          ...
          >
            <input
              ...
              autoCapitalize="none"
              className="w-full bg-[rgb(var(--fg-rgb)/5%)] border border-[rgb(var(--fg-rgb)/10%)] rounded-xl px-4 py-3 text-[rgb(var(--fg-rgb))] text-sm placeholder-[rgb(var(--fg-rgb)/30%)] outline-none focus:border-orange-700/60"
            />
            {mode === "signup" && (
              <input
                value={name}
            ...
            <button
              ...
              onClick={submit}
              disabled={busy}
              className="w-full mt-2 bg-gradient-to-r from-orange-600 to-orange-400 text-black font-semibold text-sm rounded-xl py-3 flex items-center justify-center gap-2 active:opacity-80 disabled:opacity-60"
            >
...
function CommentsModal({ item, kind, currentUser, users, onClose, onAddComment, onToggleCommentLike }) {
  ...
  const submit = async (e) => {
    ...
    setPosting(true);
    await onAddComment(item.id, t, imageUrl);
    setText("");
  ...
  const pickImage = async (e) => {
    ...
    try {
      const blob = await compressImageToBlob(file, 600, 0.75);
      const url = await uploadToMediaBucket(blob, "jpg");
      setImageUrl(url);
    } catch (err) {
  ...
  const reactWith = (emoji) => onAddComment(item.id, emoji, null);
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/70 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[85vh] flex flex-col overflow-hidden border border-[rgb(var(--fg-rgb)/10%)]">
        <div className="flex items-center justify-center relative px-4 py-3 border-b border-[rgb(var(--fg-rgb)/10%)]">
          <span className="font-semibold text-sm">Comments</span>
          <button onClick={onClose} aria-label="Close comments" className="absolute right-4"><X size={20} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          {item.comments.map((c) => {
            ...
            const isAuthor = c.user === item.username;
            return (
              <div key={c.id || c.user + c.text} className="flex gap-3">
                <img src={users[c.user]?.avatar || DEFAULT_AVATAR} alt="" className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm">
                    <span className="font-brand mr-1">{c.user}</span>
                    {c.created_at && <span className="text-[rgb(var(--fg-rgb)/35%)] text-xs mr-1">{timeAgo(c.created_at)}</span>}
                    {isAuthor && <span className="text-[rgb(var(--fg-rgb)/35%)] text-xs">· Author</span>}
                  </div>
                  {c.text && <div className="text-[rgb(var(--fg-rgb)/85%)] text-sm">{c.text}</div>}
                  {c.image && <img src={c.image} alt="" className="mt-1.5 max-w-[160px] rounded-lg object-cover" />}
                  {c.id && (
                    <button onClick={() => replyTo(c.user)} className="text-xs text-[rgb(var(--fg-rgb)/40%)] font-semibold mt-1">
                      Reply
                    </button>
                  )}
                </div>
                {c.id && (
                  <button onClick={() => onToggleCommentLike(kind, item.id, c.id)} className="flex flex-col items-center gap-0.5 pt-0.5 flex-shrink-0">
                    <Heart size={13} className={liked ? "text-red-500" : "text-[rgb(var(--fg-rgb)/30%)]"} fill={liked ? "currentColor" : "none"} />
                    {(c.likes || []).length > 0 && <span className="text-[10px] text-[rgb(var(--fg-rgb)/35%)]">{c.likes.length}</span>}
                  </button>
                )}
              </div>
            );
        ...
        <div className="flex items-center gap-3 px-4 py-2 border-t border-[rgb(var(--fg-rgb)/8%)]">
          {QUICK_REACTIONS.map((emoji) => (
            <button key={emoji} onClick={() => reactWith(emoji)} className="text-lg active:scale-125 transition-transform">
              {emoji}
            </button>
          ))}
        </div>
        ...
        {imagePreview && (
          <div className="px-4 pt-2 flex items-center gap-2">
            ...
            <button onClick={() => { setImagePreview(null); setImageUrl(null); }} className="text-xs text-[rgb(var(--fg-rgb)/40%)]">Remove</button>
          </div>
        )}
        ...
        >
          ...
          <input
            ...
            placeholder={`Add a comment for ${item.username}...`}
            className="flex-1 bg-transparent text-sm outline-none placeholder-[rgb(var(--fg-rgb)/30%)] min-w-0"
          />
          <button onClick={() => fileRef.current?.click()} aria-label="Attach photo" className="flex-shrink-0">
            <ImagePlus size={19} className="text-[rgb(var(--fg-rgb)/50%)]" />
          </button>
          <input ref={fileRef} type="file" accept="image/*" onChange={pickImage} className="hidden" />
          <button
            type="button"
            onClick={submit}
            disabled={(!text.trim() && !imageUrl) || posting || uploadingImage}
            className="text-orange-800 font-semibold text-sm disabled:text-[rgb(var(--fg-rgb)/20%)] flex-shrink-0"
          >
...
// Fallback for people outside your follow graph: still lets you share via
// the OS share sheet / clipboard. The primary path now is sending directly
// to followers/following inside the app (see ShareSheet below).
async function shareExternally(username, caption, onCopied) {
  const text = caption ? `"${caption}" — ${username} on Zgram` : `${username}'s post on Zgram`;
  const url = window.location.origin + window.location.pathname;
  if (navigator.share) {
    try {
      await navigator.share({ title: "Zgram", text, url });
    } catch (err) {
...
function PostCard({ post, currentUser, users, onToggleLike, onToggleSave, onOpenComments, onOpenProfile, onOpenShare }) {
    ...
    <article className="bg-[rgb(var(--bg-rgb))] border-b border-[rgb(var(--fg-rgb)/10%)] sm:border sm:rounded-xl mb-4 max-w-[470px] w-full mx-auto text-[rgb(var(--fg-rgb))]">
      <div className="flex items-center justify-between px-3 py-2.5">
        <button onClick={() => onOpenProfile(post.username)} className="flex items-center gap-2.5">
          ...
          <span className="font-brand text-sm">{post.username}</span>
        </button>
      ...
      <div className="flex items-center justify-between px-3 pt-2.5">
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => onOpenShare({ kind: "post", item: post })} aria-label="Share">
            <Send size={24} />
          </button>
        </div>
      ...
      <div className="px-3 pt-1 text-sm">
        <span className="font-brand mr-1">{post.username}</span>
...
function CreatePostModal({ currentUser, onClose, onCreate }) {
  const [previewLocal, setPreviewLocal] = useState(null); // instant local preview (object URL)
  const [uploadedUrl, setUploadedUrl] = useState(null); // real Storage URL, ready to post
  ...
  const pickFile = async (e) => {
    ...
    if (!file) return;
    setError("");
    setPreviewLocal(URL.createObjectURL(file)); // shows instantly, before upload finishes
    setUploadedUrl(null);
    setBusy(true);
    try {
      ...
      setError("Couldn't upload that photo — " + (err.message || "try a different one or check your connection."));
  ...
  const share = async () => {
    ...
    setSharing(true);
    await onCreate({ image: uploadedUrl, caption: caption.trim() });
    setSharing(false);
  };
    ...
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[90vh] flex flex-col overflow-hidden border border-[rgb(var(--fg-rgb)/10%)]">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[rgb(var(--fg-rgb)/10%)]">
          ...
          <button
            onClick={share}
            disabled={!uploadedUrl || busy || sharing}
            className="text-orange-800 font-semibold text-sm disabled:text-[rgb(var(--fg-rgb)/20%)] flex items-center gap-1.5"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {previewLocal ? (
            <div className="relative">
              <img src={previewLocal} alt="" className="w-full aspect-square object-cover rounded-lg" />
              {busy && (
              ...
              <button onClick={() => { setPreviewLocal(null); setUploadedUrl(null); }} className="absolute top-2 right-2 bg-black/60 rounded-full p-1.5">
                <X size={16} />
              </button>
            </div>
          ) : (
            ...
            >
              <ImagePlus size={36} />
              <span className="text-sm">Select a photo from your gallery</span>
            </button>
          )}
          {error && <p className="text-rose-400 text-xs">{error}</p>}
...
const MAX_REEL_VIDEO_MB = 50;

function CreateReelModal({ onClose, onCreate }) {
  const [videoUrl, setVideoUrl] = useState(""); // final URL, either uploaded or a sample
  const [previewLocal, setPreviewLocal] = useState(null); // instant local preview while uploading
  const [caption, setCaption] = useState("");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [sharing, setSharing] = useState(false);
  ...
  const pickVideo = async (e) => {
    ...
    if (!file) return;
    setError("");
    if (file.size > MAX_REEL_VIDEO_MB * 1024 * 1024) {
      setError(`That video is too large (max ${MAX_REEL_VIDEO_MB}MB for now) — try a shorter clip.`);
      return;
    }
    ...
    try {
      ...
      const url = await uploadToMediaBucket(file, ext);
      setVideoUrl(url);
    } catch (err) {
      console.error("reel video upload failed:", err);
      setError("Couldn't upload that video — " + (err.message || "try a different one or check your connection."));
      setPreviewLocal(null);
    }
    setBusy(false);
  };
  ...
  const share = async () => {
    if (!videoUrl) return;
    setSharing(true);
    await onCreate({ video: videoUrl, caption: caption.trim() });
    setSharing(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[90vh] overflow-y-auto p-4 border border-[rgb(var(--fg-rgb)/10%)]">
        ...
        {previewLocal ? (
          <div className="relative w-40 aspect-[9/16] rounded-lg overflow-hidden mx-auto mb-4 bg-black">
            <video src={previewLocal} className="w-full h-full object-cover" muted playsInline />
            {busy && (
            ...
            <button onClick={() => { setPreviewLocal(null); setVideoUrl(""); }} className="absolute top-1.5 right-1.5 bg-black/60 rounded-full p-1">
              <X size={14} />
            </button>
          </div>
        ) : (
          <button
            onClick={() => fileRef.current?.click()}
            className="w-full aspect-[9/16] max-h-56 mx-auto rounded-lg border-2 border-dashed border-[rgb(var(--fg-rgb)/15%)] flex flex-col items-center justify-center gap-2 text-[rgb(var(--fg-rgb)/40%)] mb-4"
          >
            <ImagePlus size={32} />
            <span className="text-sm text-center px-4">Select a video from your gallery</span>
            <span className="text-[10px] text-[rgb(var(--fg-rgb)/30%)]">Max {MAX_REEL_VIDEO_MB}MB</span>
          </button>
        )}
        <input ref={fileRef} type="file" accept="video/*" onChange={pickVideo} className="hidden" />
        {error && <p className="text-rose-400 text-xs mb-3">{error}</p>}

        <p className="text-xs text-[rgb(var(--fg-rgb)/40%)] mb-2">...or try a sample clip:</p>
        <div className="grid grid-cols-3 gap-2 mb-4">
          {SAMPLE_REEL_VIDEOS.map((v) => (
            <button
              key={v}
              onClick={() => { setVideoUrl(v); setPreviewLocal(null); setError(""); }}
              className={`aspect-[9/16] rounded-lg bg-[rgb(var(--fg-rgb)/5%)] border flex items-center justify-center text-[10px] text-[rgb(var(--fg-rgb)/40%)] px-1 text-center ${
                videoUrl === v ? "border-orange-700" : "border-[rgb(var(--fg-rgb)/10%)]"
              }`}
            >
              {v.split("/").pop()}
            </button>
        ...
        />
        <button
          disabled={!videoUrl || busy || sharing}
          onClick={share}
          className="w-full bg-gradient-to-r from-orange-600 to-orange-400 text-black rounded-lg py-2.5 text-sm font-semibold disabled:opacity-30 flex items-center justify-center gap-2"
        >
          {sharing && <Loader2 size={14} className="animate-spin" />}
          Share reel
        </button>
...
function ReelCard({ reel, currentUser, users, onToggleLike, onOpenComments, onOpenShare }) {
    ...
    <div className="h-full w-full snap-start relative bg-black flex-shrink-0">
      ...
      <div className="absolute bottom-6 left-3 right-16 text-[rgb(var(--fg-rgb))]">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-brand text-sm">{reel.username}</span>
        </div>
      ...
      <div className="absolute bottom-8 right-3 flex flex-col items-center gap-5 text-[rgb(var(--fg-rgb))]">
        ...
        <button onClick={() => onOpenShare({ kind: "reel", item: reel })} aria-label="Share">
          <Send size={24} />
        </button>
      </div>
    </div>
...
function ReelsScreen({ reels, currentUser, users, onToggleLike, onOpenComments, onCreate, onOpenShare }) {
  return (
    <div className="relative h-[calc(100dvh-56px)] lg:h-[calc(100dvh-24px)] w-full max-w-[470px] mx-auto bg-black overflow-hidden">
      ...
      {reels === null ? (
        ...
        <div className="h-full overflow-y-scroll snap-y snap-mandatory scrollbar-hide">
          {reels.map((r) => (
            <ReelCard key={r.id} reel={r} currentUser={currentUser} users={users} onToggleLike={onToggleLike} onOpenComments={onOpenComments} onOpenShare={onOpenShare} />
          ))}
        </div>
      )}
    </div>
        ...
        {list.map((u) => (
          <button key={u.username} onClick={() => onOpenProfile(u.username)} className="w-full flex items-center gap-3 py-2.5">
            ...
            <div className="text-left">
              <div className="font-brand text-sm">{u.username}</div>
...
function groupNotifications(list) {
  ...
  list.forEach((n) => {
    const ageDays = (now - new Date(n.created_at).getTime()) / 86400000;
    if (ageDays < 1) groups.Today.push(n);
    else if (ageDays < 2) groups.Yesterday.push(n);
    else if (ageDays < 7) groups["Last 7 days"].push(n);
    else groups.Earlier.push(n);
  });
  return Object.entries(groups).filter(([, items]) => items.length > 0);
}

function NotificationsScreen({ notifications, users, posts, reels, currentUser, follows, onBack, onOpenProfile, onFollowToggle }) {
  const grouped = groupNotifications(notifications);
  return (
    <div className="max-w-[470px] mx-auto text-[rgb(var(--fg-rgb))]">
      <SubScreenHeader title="Notifications" onBack={onBack} />
      {notifications.length === 0 ? (
        <p className="text-[rgb(var(--fg-rgb)/30%)] text-sm text-center py-14 px-6">No notifications yet.</p>
      ) : (
        grouped.map(([label, items]) => (
          <div key={label} className="mb-2">
            <div className="text-sm font-semibold px-4 pt-4 pb-1">{label}</div>
            {items.map((n) => {
              const actor = users[n.actor_username];
              if (!actor) return null;
              const isFollowingBack = follows.some((f) => f.follower_username === currentUser.username && f.following_username === n.actor_username);
              const relatedPost = n.post_id ? (posts || []).find((p) => p.id === n.post_id) : null;
              const relatedReel = n.reel_id ? (reels || []).find((r) => r.id === n.reel_id) : null;
              const thumb = relatedPost?.image;
              return (
                <button
                  key={n.id}
                  onClick={() => onOpenProfile(n.actor_username)}
                  className={`w-full flex items-center gap-3 px-4 py-2.5 ${!n.read ? "bg-[rgb(var(--fg-rgb)/5%)]" : ""}`}
                >
                  <img src={actor.avatar} alt="" className="w-11 h-11 rounded-full object-cover flex-shrink-0" />
                  <div className="flex-1 min-w-0 text-left">
                    <span className="text-sm">
                      <span className="font-brand">{actor.username}</span> {notificationText(n)}{" "}
                      <span className="text-[rgb(var(--fg-rgb)/35%)]">{timeAgo(n.created_at)}</span>
                    </span>
                  </div>
                  {n.type === "follow" && (
                    <span
                      onClick={(e) => { e.stopPropagation(); onFollowToggle(n.actor_username); }}
                      className={`text-xs font-semibold rounded-lg px-3 py-1.5 flex-shrink-0 ${
                        isFollowingBack ? "border border-[rgb(var(--fg-rgb)/15%)]" : "bg-gradient-to-r from-orange-600 to-orange-400 text-black"
                      }`}
                    >
                      {isFollowingBack ? "Following" : "Follow back"}
                    </span>
                  )}
                  {thumb && <img src={thumb} alt="" className="w-11 h-11 rounded object-cover flex-shrink-0" />}
                  {relatedReel && !thumb && <Film size={20} className="text-[rgb(var(--fg-rgb)/30%)] flex-shrink-0" />}
                </button>
              );
            })}
          </div>
            ...
            usernames.map((u) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-left">
                    <div className="font-brand text-sm">{profile.username}</div>
...
function ProfileScreen({ username, currentUser, users, posts, reels, follows, muted, blocked, onFollowToggle, onLogout, onEditProfile, onOpenPost, onOpenReelComments, onMessage, onOpenProfile, onOpenSettings, onMute, onBlock, onLogVisit, onOpenVisitors }) {
  const [listModal, setListModal] = useState(null); // "followers" | "following" | null
  const [showMoreMenu, setShowMoreMenu] = useState(false);
  const [gridTab, setGridTab] = useState("posts"); // "posts" | "reels"
  const user = users[username];
  const isMe = username === currentUser.username;

  // Log a visit each time someone opens a profile that isn't their own —
  // this is the data the Transparency Flip / "recent visitors" list reads.
  useEffect(() => {
    if (!isMe && user) onLogVisit(username);
  ...
  return (
    <div className="max-w-[470px] mx-auto text-[rgb(var(--fg-rgb))] px-4 pt-5">
      <div className="flex items-center justify-between mb-5">
        <span className="font-brand text-lg">{user.username}</span>
        {isMe ? (
          <div className="flex items-center gap-4">
      ...
      <div className="mb-4">
        ...
        {user.link && (
          <a href={user.link} target="_blank" rel="noreferrer" className="text-sm text-orange-800 mt-1 block truncate">
            {user.link}
          </a>
        )}
      </div>
      ...
      {isMe ? (
        <div className="flex gap-2 mb-6">
          <button onClick={onEditProfile} className="flex-1 border border-[rgb(var(--fg-rgb)/15%)] rounded-lg py-2 text-sm font-semibold">
            Edit profile
          </button>
          <button onClick={onOpenVisitors} className="flex-1 border border-[rgb(var(--fg-rgb)/15%)] rounded-lg py-2 text-sm font-semibold flex items-center justify-center gap-1.5">
            <Activity size={14} /> Visitors
          </button>
        </div>
      ) : (
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`flex-1 rounded-lg py-2 text-sm font-semibold ${isFollowing ? "border border-[rgb(var(--fg-rgb)/15%)]" : user.accent_color ? "" : "bg-gradient-to-r from-orange-600 to-orange-400 text-black"}`}
          >
      ...
      <div className="flex items-center border-t border-[rgb(var(--fg-rgb)/10%)] mt-1">
        <button
          onClick={() => setGridTab("posts")}
          className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-semibold ${
            gridTab === "posts" ? "text-[rgb(var(--fg-rgb))] border-t-2 border-[rgb(var(--fg-rgb))]" : "text-[rgb(var(--fg-rgb)/40%)] border-t-2 border-transparent"
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-semibold ${
            gridTab === "reels" ? "text-[rgb(var(--fg-rgb))] border-t-2 border-[rgb(var(--fg-rgb))]" : "text-[rgb(var(--fg-rgb)/40%)] border-t-2 border-transparent"
          }`}
        >
          <Film size={16} /> REELS
        </button>
      ...
      {gridTab === "posts" ? (
        ...
        <p className="text-[rgb(var(--fg-rgb)/30%)] text-sm text-center py-10">No reels yet.</p>
      ) : (
        <div className="grid grid-cols-3 gap-0.5">
          {myReels.map((r) => (
            <button key={r.id} onClick={() => onOpenReelComments(r)} className="relative aspect-square overflow-hidden bg-black">
              <video src={r.video} className="w-full h-full object-cover" muted playsInline preload="metadata" />
...
function ShareSheet({ target, currentUser, users, follows, onClose, onSend, onExternalShare }) {
  const [selected, setSelected] = useState([]);
  const [sending, setSending] = useState(false);
  const [sent, setSent] = useState(false);
  const [copied, setCopied] = useState(false);

  // People you can send to: anyone who follows you OR you follow — your
  // actual network, not the whole user base.
  const recipients = Array.from(
    new Set([
      ...follows.filter((f) => f.following_username === currentUser.username).map((f) => f.follower_username),
      ...follows.filter((f) => f.follower_username === currentUser.username).map((f) => f.following_username),
    ])
  ...
  const send = async () => {
    ...
    setSending(true);
    await onSend(selected, target.kind, target.item.id);
    setSending(false);
    setSent(true);
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/70 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[75vh] flex flex-col overflow-hidden border border-[rgb(var(--fg-rgb)/10%)]">
        <div className="flex items-center justify-center relative px-4 py-3 border-b border-[rgb(var(--fg-rgb)/10%)]">
          <span className="font-semibold text-sm">Send to</span>
          <button onClick={onClose} className="absolute right-4"><X size={20} /></button>
        </div>

        {sent ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-2 py-14">
            <div className="w-12 h-12 rounded-full bg-gradient-to-r from-orange-600 to-orange-400 flex items-center justify-center">
              <Send size={20} className="text-black" />
            </div>
            <p className="text-sm text-[rgb(var(--fg-rgb)/60%)]">Sent!</p>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto px-4 py-2">
              {recipients.length === 0 ? (
                <p className="text-[rgb(var(--fg-rgb)/30%)] text-sm text-center py-10">
                  Follow someone or get a follower to send things directly in Zgram.
                </p>
              ) : (
                recipients.map((username) => {
                  ...
                  return (
                    <button key={username} onClick={() => toggle(username)} className="w-full flex items-center gap-3 py-2.5">
                      <img src={u.avatar} alt="" className="w-11 h-11 rounded-full object-cover" />
                      <div className="flex-1 min-w-0 text-left">
                        <div className="font-brand text-sm truncate">{u.username}</div>
                      </div>
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                          isSelected ? "bg-orange-500 border-orange-500" : "border-[rgb(var(--fg-rgb)/25%)]"
                        }`}
                      >
            ...
            <div className="border-t border-[rgb(var(--fg-rgb)/10%)] p-3 space-y-2">
              <button
                onClick={send}
              ...
              >
                ...
                {selected.length > 0 ? `Send (${selected.length})` : "Send"}
              </button>
              <button
                onClick={() => onExternalShare(target.item.username, target.item.caption, () => { setCopied(true); setTimeout(() => setCopied(false), 1500); })}
                className="w-full border border-[rgb(var(--fg-rgb)/15%)] rounded-lg py-2.5 text-sm font-semibold"
              >
                {copied ? "Link copied" : "Share outside Zgram"}
              </button>
            </div>
          </>
        )}
...
function CreateActionSheet({ onClose, onPickPost, onPickReel }) {
  return (
    <div className="fixed inset-0 z-[60] bg-black/70 flex items-end sm:items-center justify-center animate-backdrop" onClick={onClose}>
      <div
        className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl border border-[rgb(var(--fg-rgb)/10%)] overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-center py-3 border-b border-[rgb(var(--fg-rgb)/10%)] relative">
          <span className="font-semibold text-sm">Create</span>
          <button onClick={onClose} className="absolute right-4"><X size={18} /></button>
        </div>
        <button onClick={onPickPost} className="w-full flex items-center gap-3 px-5 py-4 border-b border-[rgb(var(--fg-rgb)/8%)]">
          <Grid3x3 size={20} />
          <span className="text-sm">Post</span>
        </button>
        <button onClick={onPickReel} className="w-full flex items-center gap-3 px-5 py-4">
          <Film size={20} />
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex-1 rounded-lg py-2.5 text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-60 ${
              danger ? "bg-rose-500 text-white" : "bg-gradient-to-r from-orange-600 to-orange-400 text-black"
            }`}
          >
...
function Toggle({ checked, onChange, label, sublabel }) {
  return (
    ...
    >
      ...
      <div className={`w-11 h-6 rounded-full flex-shrink-0 flex items-center px-0.5 transition-colors ${checked ? "bg-orange-600 justify-end" : "bg-[rgb(var(--fg-rgb)/15%)] justify-start"}`}>
...
const ACCENT_PRESETS = [
  { name: "Classic", from: "#ea580c", to: "#1c1917" }, // new brand default: orange -> near-black
...
// Every place that used the hardcoded from-orange-600 to-orange-400 brand
    ...
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[90vh] overflow-y-auto p-4 border border-[rgb(var(--fg-rgb)/10%)]">
        ...
        <div className="flex flex-col items-center mb-5">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading} className="text-orange-800 text-sm font-semibold flex items-center gap-1.5">
        ...
        <div className="border-t border-[rgb(var(--fg-rgb)/10%)] pt-3 mt-1 mb-4">
          ...
          <div className="flex gap-3 flex-wrap">
            <button
              ...
              className={`w-9 h-9 rounded-full bg-gradient-to-r from-orange-600 to-orange-400 ${accentColor === "" ? "ring-2 ring-offset-2 ring-offset-[rgb(var(--bg-rgb))] ring-[rgb(var(--fg-rgb))]" : ""}`}
        ...
        <button
          ...
          disabled={saving}
          className="w-full bg-gradient-to-r from-orange-600 to-orange-400 text-black rounded-lg py-2.5 text-sm font-semibold disabled:opacity-60 flex items-center justify-center gap-2"
        >
  ...
  return (
    <div>
      ...
      <div className="px-4">
        ...
        />
        <Toggle
          checked={currentUser.show_activity !== false}
          onChange={(v) => onUpdate({ show_activity: v })}
          label="Show my activity"
          sublabel="When on, visiting someone's profile logs a visit they can see in their Visitors list. Turn off to browse without leaving a trace."
        />
      </div>
    </div>
  );
...
function VisitorsScreen({ visitors, users, onBack, onOpenProfile }) {
  return (
    <div>
      <SubScreenHeader title="Recent visitors" onBack={onBack} />
      <p className="text-xs text-[rgb(var(--fg-rgb)/40%)] px-4 pt-3">
        Most social apps hide this. Zgram shows it to you — people who visited while their activity sharing was on.
      </p>
      <div className="px-4 mt-2">
        {visitors.length === 0 ? (
          <p className="text-[rgb(var(--fg-rgb)/40%)] text-sm text-center py-14">No visits yet.</p>
        ) : (
          visitors.map((v) => (
            <button key={v.id} onClick={() => onOpenProfile(v.viewer_username)} className="w-full">
              <UserListRow
                username={v.viewer_username}
                users={users}
                right={<span className="text-xs text-[rgb(var(--fg-rgb)/35%)] flex-shrink-0">{timeAgo(v.created_at)} ago</span>}
              />
            </button>
          ))
        )}
...
function LoginHistoryScreen({ history, onBack }) {
  return (
    <div>
      <SubScreenHeader title="Login activity" onBack={onBack} />
      <p className="text-xs text-[rgb(var(--fg-rgb)/40%)] px-4 pt-3">Every time your account has signed in, visible only to you.</p>
      <div className="px-4 mt-2 divide-y divide-[rgb(var(--fg-rgb)/8%)]">
        {history.length === 0 ? (
          <p className="text-[rgb(var(--fg-rgb)/40%)] text-sm text-center py-14">No login history yet.</p>
        ) : (
          history.map((h) => (
            <div key={h.id} className="py-3">
              <div className="text-sm">{h.device_info || "Unknown device"}</div>
              <div className="text-xs text-[rgb(var(--fg-rgb)/40%)]">{new Date(h.created_at).toLocaleString()}</div>
            </div>
          ))
        )}
      </div>
    ...
    <div>
      ...
      <div className="px-4 pt-2 space-y-2">
        <button
          ...
          className={`w-full flex items-center gap-3 p-3 rounded-lg border ${theme === "dark" ? "border-orange-700" : "border-[rgb(var(--fg-rgb)/10%)]"}`}
        >
          <Moon size={20} />
          <span className="text-sm flex-1 text-left">Dark</span>
          {theme === "dark" && <BadgeCheck size={18} className="text-orange-800" />}
        </button>
        <button
          onClick={() => onUpdate({ theme: "light" })}
          className={`w-full flex items-center gap-3 p-3 rounded-lg border ${theme === "light" ? "border-orange-700" : "border-[rgb(var(--fg-rgb)/10%)]"}`}
        >
          <Sun size={20} />
          <span className="text-sm flex-1 text-left">Light</span>
          {theme === "light" && <BadgeCheck size={18} className="text-orange-800" />}
        </button>
      </div>
    </div>
  );
...
function SettingsScreen({
  currentUser, users, posts, muted, blocked, closeFriends, visitors, loginHistory,
  onBack, onUpdateUser, onLogout, onOpenSaved, onOpenSearch, onOpenProfile,
  ...
  if (screen === "visitors") return <VisitorsScreen visitors={visitors} users={users} onBack={() => setScreen("root")} onOpenProfile={onOpenProfile} />;
  if (screen === "loginhistory") return <LoginHistoryScreen history={loginHistory} onBack={() => setScreen("root")} />;
  ...
  return (
    <div className="max-w-[470px] mx-auto text-[rgb(var(--fg-rgb))] pb-10">
      ...
      <SettingsSection title="Transparency (Zgram's take on privacy)">
        <SettingsRow icon={Activity} label="Recent visitors" sublabel="See who's viewed your profile" value={visitors.length} onClick={() => setScreen("visitors")} />
        <SettingsRow icon={ShieldCheck} label="Login activity" sublabel="Every sign-in to your account" onClick={() => setScreen("loginhistory")} />
      </SettingsSection>
...
function DMListScreen({ currentUser, users, partners, onOpenChat, onNewMessage, onlineUsernames }) {
  const [showNew, setShowNew] = useState(false);
  return (
    <div className="max-w-[470px] mx-auto text-[rgb(var(--fg-rgb))]">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[rgb(var(--fg-rgb)/10%)]">
        <span className="font-brand">{currentUser.username}</span>
      ...
      {partners.length === 0 ? (
        ...
        <div className="px-2">
          {partners.map((p) => {
            ...
            if (!u) return null;
            const online = onlineUsernames.has(p.username);
            return (
              <button
                key={p.username}
                onClick={() => onOpenChat(p.username)}
                className={`w-full flex items-center gap-3 px-2 py-2.5 rounded-lg transition-colors ${p.unread ? "bg-[rgb(var(--fg-rgb)/5%)]" : ""}`}
              >
                <div className="relative flex-shrink-0">
                  <img src={u.avatar} alt="" className="w-12 h-12 rounded-full object-cover" />
                  <span className="absolute -bottom-0.5 -right-0.5 text-xs bg-[rgb(var(--bg-rgb))] rounded-full leading-none p-0.5">
                    {online ? "🪐" : "🌙"}
                  </span>
                </div>
                <div className="text-left flex-1 min-w-0">
                  <div className="font-brand text-sm truncate">{u.username}</div>
                  <div className={`text-xs truncate ${p.unread ? "text-[rgb(var(--fg-rgb)/80%)] font-semibold" : "text-[rgb(var(--fg-rgb)/40%)]"}`}>
                    {p.lastMessage}
                  </div>
                </div>
                {p.unread && <span className="w-2.5 h-2.5 rounded-full bg-orange-600 flex-shrink-0" />}
              </button>
            );
          })}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3">
          {list.map((u) => (
            <button key={u.username} onClick={() => onPick(u.username)} className="w-full flex items-center gap-3 py-2.5">
              <img src={u.avatar} alt="" className="w-10 h-10 rounded-full object-cover" />
              <span className="font-brand text-sm">{u.username}</span>
            </button>
          ))}
        </div>
      </div>
...
function ChatScreen({ currentUser, otherUsername, users, onBack, onlineUsernames }) {
  const [messages, setMessages] = useState(null);
  const [text, setText] = useState("");
  const [sending, setSending] = useState(false);
  const [sharedItems, setSharedItems] = useState({}); // "post-<id>" | "reel-<id>" -> row
  ...
  // Shared posts/reels are referenced by id on the message row — fetch
  // whichever ones we haven't already got, so the bubble can show a real
  // thumbnail/caption instead of just "Sent a post".
  useEffect(() => {
    if (!messages) return;
    const missingPostIds = [...new Set(messages.filter((m) => m.shared_post_id && !sharedItems[`post-${m.shared_post_id}`]).map((m) => m.shared_post_id))];
    const missingReelIds = [...new Set(messages.filter((m) => m.shared_reel_id && !sharedItems[`reel-${m.shared_reel_id}`]).map((m) => m.shared_reel_id))];
    if (missingPostIds.length === 0 && missingReelIds.length === 0) return;
    (async () => {
      const updates = {};
      if (missingPostIds.length > 0) {
        const { data } = await supabase.from("posts").select("id, username, image, caption").in("id", missingPostIds);
        (data || []).forEach((row) => { updates[`post-${row.id}`] = row; });
      }
      if (missingReelIds.length > 0) {
        const { data } = await supabase.from("reels").select("id, username, video, caption").in("id", missingReelIds);
        (data || []).forEach((row) => { updates[`reel-${row.id}`] = row; });
      }
      setSharedItems((prev) => ({ ...prev, ...updates }));
    })();
  }, [messages, sharedItems]);

  // Mark any messages from the other person as read — called on load and
  // whenever a fresh one arrives while this chat is open.
  const markRead = useCallback(async (list) => {
    const unread = list.filter((m) => m.sender_username !== currentUser.username && !m.read_at);
    if (unread.length === 0) return;
    const now = new Date().toISOString();
    setMessages((prev) => (prev || []).map((m) => (unread.some((u) => u.id === m.id) ? { ...m, read_at: now } : m)));
  ...
  useEffect(() => {
    ...
    // Realtime replaces polling here: Postgres pushes new rows (and changes,
    // like read receipts) over a websocket instantly, instead of us asking
    // "anything new?" every few seconds. Scoped to this conversation only.
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const isFirstRender = !hasScrolledOnceRef.current;
    // requestAnimationFrame ensures the new message DOM has actually been
    // laid out before we measure scrollHeight — measuring too early was
    // why the chat sometimes opened anchored partway up instead of at the
    // newest message.
    requestAnimationFrame(() => {
      if (isFirstRender) {
        el.scrollTop = el.scrollHeight; // instant jump straight to the bottom on open
  ...
  return (
    <div className="max-w-[470px] mx-auto text-[rgb(var(--fg-rgb))] flex flex-col h-[calc(100dvh-56px)] lg:h-[calc(100dvh-24px)]">
      <div className="flex items-center gap-3 px-3 py-3 border-b border-[rgb(var(--fg-rgb)/10%)]">
        ...
        <div className="flex-1">
          <div className="font-brand text-sm">{other.username}</div>
          <div className="flex items-center gap-1">
            <span className="text-xs">{onlineUsernames.has(otherUsername) ? "🪐" : "🌙"}</span>
            <span className="text-[11px] text-[rgb(var(--fg-rgb)/35%)]">{onlineUsernames.has(otherUsername) ? "Online" : "Offline"}</span>
          </div>
        </div>
      </div>
      <div ref={containerRef} className="flex-1 overflow-y-auto px-3 py-3 flex flex-col justify-end min-h-0">
        ...
        {messages === null ? (
          ...
          messages.map((m, i) => {
            ...
            const shared = m.shared_post_id ? sharedItems[`post-${m.shared_post_id}`] : m.shared_reel_id ? sharedItems[`reel-${m.shared_reel_id}`] : null;
            return (
              <div key={m.id} className={`flex flex-col ${mine ? "items-end" : "items-start"} ${prevSameSender ? "mt-0.5" : "mt-2.5"}`}>
                {shared ? (
                  <div className={`max-w-[75%] rounded-2xl overflow-hidden animate-msg-in border border-[rgb(var(--fg-rgb)/10%)] ${mine ? "origin-bottom-right" : "origin-bottom-left"}`}>
                    {m.shared_post_id ? (
                      <img src={shared.image} alt="" className="w-full max-h-56 object-cover" />
                    ) : (
                      <video src={shared.video} className="w-full max-h-56 object-cover" muted playsInline />
                    )}
                    <div className="px-3 py-2 bg-[rgb(var(--fg-rgb)/5%)]">
                      <div className="font-brand text-xs">{shared.username}</div>
                      {shared.caption && <div className="text-xs text-[rgb(var(--fg-rgb)/60%)] truncate">{shared.caption}</div>}
                    </div>
                  </div>
                ) : (
                  <div
                    className={`max-w-[75%] rounded-2xl px-3.5 py-2 text-sm animate-msg-in ${
                      mine ? "bg-gradient-to-r from-orange-600 to-orange-400 text-black origin-bottom-right" : "bg-[rgb(var(--fg-rgb)/10%)] origin-bottom-left"
                    }`}
                  >
                    {m.text}
                  </div>
                ...
                {isLastMine && (
                  <span className="text-[10px] text-[rgb(var(--fg-rgb)/35%)] mt-0.5 mr-1">
                    {m.read_at ? "Seen" : "Sent"}
                  </span>
                )}
              </div>
      ...
      <div className="flex items-center gap-2 border-t border-[rgb(var(--fg-rgb)/10%)] px-3 py-2.5" onKeyDown={(e) => { if (e.key === "Enter") send(e); }}>
        ...
        <button type="button" onClick={send} disabled={!text.trim() || sending} className="text-orange-800 disabled:text-[rgb(var(--fg-rgb)/20%)]">
...
function BottomNav({ tab, setTab, avatarUrl }) {
  const order = ["home", "reels", "dms", "search", "profile"];
  const activeIndex = Math.max(0, order.indexOf(tab));
  const items = [
    { key: "home", icon: Home },
  ...
  return (
    <nav className="lg:hidden fixed bottom-4 left-4 right-4 z-30">
      <div className="relative grid grid-cols-5 items-center rounded-[28px] backdrop-blur-2xl bg-[rgb(var(--bg-rgb)/55%)] border border-[rgb(var(--fg-rgb)/12%)] shadow-2xl shadow-black/40 py-3 overflow-hidden">
        {/* frosted glass sheen */}
        <div className="absolute inset-0 bg-gradient-to-b from-[rgb(var(--fg-rgb)/8%)] to-transparent pointer-events-none" />
        {/* sliding glow behind the active tab */}
        <div
          className="absolute top-1.5 bottom-1.5 w-1/5 rounded-full bg-gradient-to-br from-orange-500/30 via-orange-400/20 to-transparent transition-all duration-300 ease-out"
          style={{ left: `${activeIndex * 20}%` }}
        />
        {items.map(({ key, icon: Icon }) => {
          const active = tab === key;
          return (
            <button key={key} onClick={() => setTab(key)} className="relative z-10 flex items-center justify-center">
              <Icon
                size={25}
                className={`transition-all duration-300 ${
                  active
                    ? "text-[rgb(var(--fg-rgb))] scale-110 drop-shadow-[0_0_8px_rgba(168,85,247,0.65)]"
                    : "text-[rgb(var(--fg-rgb)/40%)] scale-100"
                }`}
              />
            </button>
          );
        })}
        <button onClick={() => setTab("profile")} aria-label="Profile" className="relative z-10 flex items-center justify-center">
          <img
            src={avatarUrl}
            alt=""
            className={`rounded-full object-cover transition-all duration-300 ${
              tab === "profile"
                ? "w-7 h-7 ring-2 ring-orange-700 shadow-[0_0_10px_rgba(168,85,247,0.7)]"
                : "w-6 h-6 ring-1 ring-[rgb(var(--fg-rgb)/25%)]"
            }`}
          />
        </button>
      </div>
...
function SideNav({ tab, setTab, onLogout, hasUnreadNotifications, onOpenNotifications }) {
  const items = [
  ...
  return (
    <nav className="hidden lg:flex flex-col gap-1 w-60 shrink-0 py-6 px-3 text-[rgb(var(--fg-rgb))]">
      <div className="font-brand px-3 pb-6 text-3xl bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent">Z</div>
      ...
      <button onClick={onOpenNotifications} className={`relative flex items-center gap-4 px-3 py-2.5 rounded-lg hover:bg-[rgb(var(--fg-rgb)/5%)] text-left ${tab === "notifications" ? "font-bold" : "text-[rgb(var(--fg-rgb)/70%)]"}`}>
        <Bell size={24} />
        {hasUnreadNotifications && <span className="absolute top-2 left-6 w-2 h-2 rounded-full bg-rose-500" />}
...
export default function App() {
  ...
  // Presence: everyone logged in joins the same shared channel and "tracks"
  // themselves; Supabase Realtime keeps every client's view of who's
  // currently present in sync, no polling needed for this at all.
  useEffect(() => {
    if (!currentUser) {
    ...
    channel
      .on("presence", { event: "sync" }, () => {
        setOnlineUsernames(new Set(Object.keys(channel.presenceState())));
      })
      .subscribe((status) => {
  ...
  const loadDmPartners = useCallback(async (myUsername) => {
    const { data } = await supabase
      .from("messages")
      .select("sender_username, receiver_username, text, read_at, created_at")
      .or(`sender_username.eq.${myUsername},receiver_username.eq.${myUsername}`)
      .order("created_at", { ascending: true });

    const byPartner = new Map();
    (data || []).forEach((m) => {
      const partner = m.sender_username === myUsername ? m.receiver_username : m.sender_username;
      const unreadFromThem = m.sender_username === partner && !m.read_at;
      const existing = byPartner.get(partner) || { username: partner, lastMessage: "", lastAt: null, unread: false };
      existing.lastMessage = m.text;
      existing.lastAt = m.created_at;
      // Once true, stays true until they open the chat and markRead runs —
      // a later *read* message from them shouldn't flip it back off here.
      if (unreadFromThem) existing.unread = true;
      byPartner.set(partner, existing);
    });

    const list = Array.from(byPartner.values()).sort((a, b) => new Date(b.lastAt) - new Date(a.lastAt));
    setDmPartners(list);
  }, []);
  ...
  const loadVisitors = useCallback(async () => {
    const { data } = await supabase.from("profile_views").select("*").order("created_at", { ascending: false }).limit(100);
    setVisitors(data || []);
  }, []);
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      loadProfile();
      // Only log a real sign-in, not every token refresh / tab focus event
      // this listener also fires for.
      if (event === "SIGNED_IN" && session) {
        const deviceInfo = typeof navigator !== "undefined" ? navigator.userAgent.slice(0, 120) : "";
  ...
  useEffect(() => {
    ...
    // The DM conversation list (order + bold/unread) should update the
    // instant a message arrives anywhere, not wait for the next 6s poll.
    const dmListChannel = supabase
      .channel(`dm-list:${currentUser.username}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "messages", filter: `receiver_username=eq.${currentUser.username}` },
        () => loadDmPartners(currentUser.username)
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "messages", filter: `sender_username=eq.${currentUser.username}` },
        () => loadDmPartners(currentUser.username)
      )
      .subscribe();

    // Notifications badge updates instantly too, same reasoning.
    ...
    return () => {
      ...
      supabase.removeChannel(dmListChannel);
      supabase.removeChannel(notifChannel);
    };
  }, [currentUser, loadAllUsers, loadPosts, loadReels, loadDmPartners, loadFollows, loadMuted, loadBlocked, loadCloseFriends, loadVisitors, loadLoginHistory, loadNotifications]);
  ...
  // Fires a notification into someone else's inbox. Never notifies yourself
  // (liking/commenting on your own stuff), and callers decide whether an
  // action deserves one (e.g. only on follow, not unfollow).
  const notify = async (recipientUsername, type, extra = {}) => {
  ...
  const toggleLikePost = async (id) => {
    ...
    if (!has) notify(post.username, "like_post", { post_id: id });
  };
  ...
  const addCommentToPost = async (id, text, image) => {
    const post = (posts || []).find((p) => p.id === id);
    if (!post) return;
    const comment = { id: Date.now() + "-" + Math.random().toString(36).slice(2), user: currentUser.username, text, image: image || null, created_at: new Date().toISOString(), likes: [] };
    ...
    notify(post.username, "comment_post", { post_id: id });
  };
  ...
  const toggleLikeReel = async (id) => {
    ...
    if (!has) notify(reel.username, "like_reel", { reel_id: id });
  };

  const addCommentToReel = async (id, text, image) => {
    const reel = (reels || []).find((r) => r.id === id);
    if (!reel) return;
    const comment = { id: Date.now() + "-" + Math.random().toString(36).slice(2), user: currentUser.username, text, image: image || null, created_at: new Date().toISOString(), likes: [] };
    const nextComments = [...reel.comments, comment];
    ...
    notify(reel.username, "comment_reel", { reel_id: id });
  };

  const toggleCommentLike = async (kind, itemId, commentId) => {
    const table = kind === "post" ? "posts" : "reels";
    const list = kind === "post" ? posts : reels;
    const setList = kind === "post" ? setPosts : setReels;
    const item = (list || []).find((x) => x.id === itemId);
    if (!item) return;
    const nextComments = item.comments.map((c) => {
      if (c.id !== commentId) return c;
      const likes = c.likes || [];
      const has = likes.includes(currentUser.username);
      return { ...c, likes: has ? likes.filter((u) => u !== currentUser.username) : [...likes, currentUser.username] };
    });
    setList((prev) => prev.map((x) => (x.id === itemId ? { ...x, comments: nextComments } : x)));
    setCommentsFor((c) => (c ? { ...c, item: { ...c.item, comments: nextComments } } : c));
    await supabase.from(table).update({ comments: nextComments }).eq("id", itemId);
  };
  ...
  const toggleFollow = async (targetUsername) => {
    ...
    if (isFollowing) {
      ...
      else notify(targetUsername, "follow");
    }
    loadFollows();
  };
  ...
  // Sends a post/reel as a message to each selected recipient — this is
  // what makes the shared item actually show up in their DMs, not just an
  // external link that goes nowhere useful.
  const sendSharedItem = async (recipientUsernames, kind, itemId) => {
    const { data: sessionData } = await supabase.auth.getSession();
    const myId = sessionData.session.user.id;
    const rows = recipientUsernames.map((username) => {
      const recipient = users[username];
      const cid = convId(currentUser.username, username);
      return {
        conversation_id: cid,
        sender_id: myId,
        sender_username: currentUser.username,
        ...
        text: kind === "post" ? "Sent a post" : "Sent a reel",
        shared_post_id: kind === "post" ? itemId : null,
        shared_reel_id: kind === "reel" ? itemId : null,
      };
  ...
  // The core of the Transparency Flip feature: log that currentUser looked
  // at targetUsername's profile, unless they've turned that off for
  // themselves in Privacy settings.
  const logVisit = async (targetUsername) => {
    if (currentUser.show_activity === false) return;
    const target = users[targetUsername];
    if (!target || target.user_id === currentUser.user_id) return;
    await supabase.from("profile_views").insert({
      viewer_id: currentUser.user_id,
      viewer_username: currentUser.username,
      viewed_id: target.user_id,
      viewed_username: targetUsername,
    });
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[rgb(var(--bg-rgb))] flex flex-col items-center justify-center gap-4">
        <span className="font-brand text-3xl bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent animate-pulse">
          Zgram
        </span>
  ...
  return (
    <div className="min-h-screen bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] font-sans pb-20 lg:pb-0">
      <header className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-[rgb(var(--fg-rgb)/10%)] bg-[rgb(var(--bg-rgb))] sticky top-0 z-20">
        <span className="font-brand text-2xl bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent">Z</span>
        <div className="flex items-center gap-4">
          <button onClick={() => setShowCreateSheet(true)} aria-label="Create"><PlusSquare size={22} /></button>
          <button onClick={() => { setTab("notifications"); markNotificationsRead(); }} aria-label="Notifications" className="relative">
            <Bell size={22} />
      ...
      <div className="flex max-w-[935px] mx-auto">
        <SideNav
          ...
          onOpenNotifications={() => { setTab("notifications"); markNotificationsRead(); }}
        />
        ...
        <main className="flex-1 min-w-0 pt-0 lg:pt-6 px-0 sm:px-4">
                  ...
                  {(() => {
                    ...
                    return visiblePosts.length === 0 ? (
                      ...
                      visiblePosts.map((post) => (
                        <PostCard key={post.id} post={post} currentUser={currentUser} users={users} onToggleLike={toggleLikePost} onToggleSave={toggleSavePost} onOpenComments={openPost} onOpenProfile={openProfile} onOpenShare={setShareTarget} />
                      ))
                    );
                  })()}
          ...
          {tab === "reels" && (
            <FadeIn tKey="reels">
              <ReelsScreen reels={reels} currentUser={currentUser} users={users} onToggleLike={toggleLikeReel} onOpenComments={openReelComments} onCreate={() => setShowCreateReel(true)} onOpenShare={setShareTarget} />
            </FadeIn>
          )}
          ...
          {tab === "dms" && !chatWith && (
            <FadeIn tKey="dms-list">
              <DMListScreen currentUser={currentUser} users={users} partners={dmPartners} onOpenChat={openChat} onNewMessage={openChat} onlineUsernames={onlineUsernames} />
            </FadeIn>
          )}
          {tab === "dms" && chatWith && (
            <FadeIn tKey={"chat-" + chatWith}>
              <ChatScreen currentUser={currentUser} otherUsername={chatWith} users={users} onBack={() => setChatWith(null)} onlineUsernames={onlineUsernames} />
            </FadeIn>
          )}
          ...
          {tab === "profile" && (
            <FadeIn tKey="my-profile">
              <ProfileScreen
                username={currentUser.username} currentUser={currentUser} users={users} posts={posts} reels={reels} follows={follows}
                ...
                onOpenPost={openPost} onOpenReelComments={openReelComments} onMessage={openChat} onOpenProfile={openProfile}
                onOpenSettings={() => setTab("settings")} onMute={muteUser} onBlock={blockUser}
                onLogVisit={logVisit} onOpenVisitors={() => setTab("settings")}
              />
            </FadeIn>
          )}
          ...
          {tab === "viewProfile" && viewProfile && (
            <FadeIn tKey={"profile-" + viewProfile}>
              <ProfileScreen
                username={viewProfile} currentUser={currentUser} users={users} posts={posts} reels={reels} follows={follows}
                muted={muted} blocked={blocked}
                onFollowToggle={toggleFollow} onLogout={() => setShowLogoutConfirm(true)} onEditProfile={() => setShowEditProfile(true)}
                onOpenPost={openPost} onOpenReelComments={openReelComments} onMessage={(u) => { setTab("dms"); openChat(u); }} onOpenProfile={openProfile}
          ...
          {tab === "settings" && (
            <FadeIn tKey="settings">
              <SettingsScreen
                ...
                visitors={visitors} loginHistory={loginHistory}
          ...
          {tab === "notifications" && (
            <FadeIn tKey="notifications">
              <NotificationsScreen
                notifications={notifications} users={users} posts={posts} reels={reels} currentUser={currentUser} follows={follows}
                onBack={() => setTab("home")}
                onOpenProfile={openProfile}
      ...
      {!(tab === "dms" && chatWith) && (
        <BottomNav tab={["viewProfile", "settings"].includes(tab) ? "profile" : tab} setTab={(t) => { setTab(t); setChatWith(null); }} avatarUrl={currentUser.avatar} />
      )}

      {commentsFor && (
        <CommentsModal
          item={commentsFor.item}
          kind={commentsFor.type}
          currentUser={currentUser}
          users={users}
      ...
      {showCreateSheet && (
        <CreateActionSheet
          ...
          onPickPost={() => { setShowCreateSheet(false); setShowCreatePost(true); }}
          onPickReel={() => { setShowCreateSheet(false); setShowCreateReel(true); }}
        />
      )}
