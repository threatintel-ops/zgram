...
function AuthScreen({ onAuthed }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-black via-neutral-950 to-orange-950 px-4">
      <div className="w-full max-w-sm">
        <div className="bg-[rgb(var(--fg-rgb)/4%)] backdrop-blur-xl border border-[rgb(var(--fg-rgb)/10%)] rounded-3xl p-8">
          <h1 className="text-center text-6xl font-black tracking-tight mb-1 bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent">
            Z
          </h1>
          ...
          >
            ...
            <button
              ...
              onClick={submit}
              disabled={busy}
              className="w-full mt-2 bg-gradient-to-r from-orange-600 to-orange-400 text-[rgb(var(--fg-rgb))] font-semibold text-sm rounded-xl py-3 flex items-center justify-center gap-2 active:opacity-80 disabled:opacity-60"
            >
...
// Zgram is a single-page app with no per-post routing, so there's no real
// deep link to a specific post/reel to share yet — this shares the app
// itself plus a text blurb identifying the post, which is honest about
// what's actually shareable right now. Uses the native share sheet where
// available (basically all mobile browsers), falls back to copying a link.
async function shareItem(username, caption, onCopied) {
      ...
      <div className="flex items-center justify-between px-3 pt-2.5">
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => shareItem(post.username, post.caption, () => { setCopied(true); setTimeout(() => setCopied(false), 1500); })} aria-label="Share">
              <Send size={24} />
            </button>
            {copied && (
              <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[rgb(var(--fg-rgb))] text-[rgb(var(--bg-rgb))] text-[11px] font-semibold px-2 py-1 rounded-md whitespace-nowrap">
                Link copied
              </span>
            )}
          </div>
    ...
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[90vh] overflow-y-auto p-4 border border-[rgb(var(--fg-rgb)/10%)]">
        ...
        <button
          ...
          onClick={share}
          className="w-full bg-gradient-to-r from-orange-600 to-orange-400 rounded-lg py-2.5 text-sm font-semibold disabled:opacity-30 flex items-center justify-center gap-2"
        >
          {sharing && <Loader2 size={14} className="animate-spin" />}
          Share reel
        </button>
  ...
  return (
    <div className="h-full w-full snap-start relative bg-black flex-shrink-0">
      ...
      <div className="absolute bottom-8 right-3 flex flex-col items-center gap-5 text-[rgb(var(--fg-rgb))]">
        ...
        <div className="relative">
          <button onClick={() => shareItem(reel.username, reel.caption, () => { setCopied(true); setTimeout(() => setCopied(false), 1500); })} aria-label="Share">
            <Send size={24} />
          </button>
          {copied && (
            <span className="absolute top-1/2 -translate-y-1/2 right-9 bg-white text-black text-[11px] font-semibold px-2 py-1 rounded-md whitespace-nowrap">
              Link copied
            </span>
          )}
        </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <span className="text-sm">
                      <span className="font-semibold">{actor.username}</span> {notificationText(n)}{" "}
...
function ProfileScreen({ username, currentUser, users, posts, follows, muted, blocked, onFollowToggle, onLogout, onEditProfile, onOpenPost, onMessage, onOpenProfile, onOpenSettings, onMute, onBlock, onLogVisit, onOpenVisitors }) {
      ...
      {isMe ? (
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`flex-1 rounded-lg py-2 text-sm font-semibold ${isFollowing ? "border border-[rgb(var(--fg-rgb)/15%)]" : user.accent_color ? "" : "bg-gradient-to-r from-orange-600 to-orange-400"}`}
          >
    ...
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center animate-backdrop">
      <div className="animate-sheet bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] w-full sm:max-w-md sm:rounded-xl max-h-[90vh] overflow-y-auto p-4 border border-[rgb(var(--fg-rgb)/10%)]">
        ...
        <button
          ...
          disabled={saving}
          className="w-full bg-gradient-to-r from-orange-600 to-orange-400 rounded-lg py-2.5 text-sm font-semibold disabled:opacity-60 flex items-center justify-center gap-2"
        >
            ...
            return (
              ...
              >
                ...
                <div className="text-left flex-1 min-w-0">
                  <div className={`text-sm truncate ${p.unread ? "font-bold" : "font-semibold"}`}>{u.username}</div>
              ...
              <div key={m.id} className={`flex flex-col ${mine ? "items-end" : "items-start"} ${prevSameSender ? "mt-0.5" : "mt-2.5"}`}>
                <div
                  className={`max-w-[75%] rounded-2xl px-3.5 py-2 text-sm animate-msg-in ${
                    mine ? "bg-gradient-to-r from-orange-600 to-orange-400 origin-bottom-right" : "bg-[rgb(var(--fg-rgb)/10%)] origin-bottom-left"
                  }`}
                >
                  {m.text}
                </div>
...
function SideNav({ tab, setTab, onLogout, hasUnreadNotifications, onOpenNotifications }) {
  ...
  return (
    <nav className="hidden lg:flex flex-col gap-1 w-60 shrink-0 py-6 px-3 text-[rgb(var(--fg-rgb))]">
      <div className="px-3 pb-6 text-3xl font-black bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent">Z</div>
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[rgb(var(--bg-rgb))] flex flex-col items-center justify-center gap-4">
        <span className="text-3xl font-black bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent animate-pulse">
          Zgram
        </span>
  ...
  return (
    <div className="min-h-screen bg-[rgb(var(--bg-rgb))] text-[rgb(var(--fg-rgb))] font-sans pb-20 lg:pb-0">
      <header className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-[rgb(var(--fg-rgb)/10%)] bg-[rgb(var(--bg-rgb))] sticky top-0 z-20">
        <span className="text-2xl font-black bg-gradient-to-r from-neutral-900 via-orange-700 to-orange-400 bg-clip-text text-transparent">Z</span>
        <div className="flex items-center gap-4">
