...
function CommentsModal({ item, currentUser, users, onClose, onAddComment }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/70 flex items-end sm:items-center justify-center">
      <div className="bg-neutral-950 text-white w-full sm:max-w-md sm:rounded-xl max-h-[85vh] flex flex-col overflow-hidden border border-white/10">
  ...
  return (
    <article className="bg-neutral-950 border-b border-white/10 sm:border sm:rounded-xl mb-4 max-w-[470px] w-full mx-auto text-white">
      ...
      <div className="relative select-none" onClick={handleTap}>
        <img src={post.image} alt="" className="w-full aspect-square object-cover" draggable={false} />
...
function CreatePostModal({ currentUser, onClose, onCreate }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center">
      <div className="bg-neutral-950 text-white w-full sm:max-w-md sm:rounded-xl max-h-[90vh] flex flex-col overflow-hidden border border-white/10">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
...
function CreateReelModal({ onClose, onCreate }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center">
      <div className="bg-neutral-950 text-white w-full sm:max-w-md sm:rounded-xl p-4 border border-white/10">
...
function ProfileScreen({ username, currentUser, users, posts, onFollowToggle, onLogout, onEditProfile, onOpenPost, onMessage }) {
    ...
    <div className="max-w-[470px] mx-auto text-white px-4 pt-5">
      ...
      <div className="flex items-center gap-6 mb-4">
        ...
        <div className="flex-1 grid grid-cols-3 text-center">
          ...
          <div><div className="font-semibold">{(user.followers || []).length}</div><div className="text-xs text-white/40">Followers</div></div>
          <div><div className="font-semibold">{(user.following || []).length}</div><div className="text-xs text-white/40">Following</div></div>
        </div>
      </div>
      <div className="mb-4">
...
function NewMessageModal({ currentUser, users, onClose, onPick }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center">
      <div className="bg-neutral-950 text-white w-full sm:max-w-md sm:rounded-xl max-h-[80vh] flex flex-col border border-white/10">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [currentUser, loadAllUsers, loadPosts, loadReels, loadDmPartners]);
  ...
  const toggleFollow = async (targetUsername) => {
    ...
    if (!target) return;
    const isFollowing = (target.followers || []).includes(me.username);
    ...
    const nextMyFollowing = isFollowing
      ? (me.following || []).filter((u) => u !== targetUsername)
      : [...(me.following || []), targetUsername];

    setUsers((prev) => ({ ...prev, [targetUsername]: { ...target, followers: nextTargetFollowers } }));
    setCurrentUser((prev) => ({ ...prev, following: nextMyFollowing }));

    await supabase.from("profiles").update({ followers: nextTargetFollowers }).eq("user_id", target.user_id);
    await supabase.from("profiles").update({ following: nextMyFollowing }).eq("user_id", me.user_id);
  };
    ...
    <div className="min-h-screen bg-neutral-950 text-white font-sans pb-14 lg:pb-0">
      ...
      <div className="flex max-w-[935px] mx-auto">
        ...
        <main className="flex-1 min-w-0 pt-0 lg:pt-6 px-0 sm:px-4">
          ...
          {tab === "search" && <SearchScreen users={users} currentUser={currentUser} onOpenProfile={openProfile} />}
          ...
          {tab === "profile" && (
            <ProfileScreen username={currentUser.username} currentUser={currentUser} users={users} posts={posts} onFollowToggle={toggleFollow} onLogout={handleLogout} onEditProfile={() => setShowEditProfile(true)} onOpenPost={openPost} onMessage={openChat} />
          )}

          {tab === "viewProfile" && viewProfile && (
            <ProfileScreen username={viewProfile} currentUser={currentUser} users={users} posts={posts} onFollowToggle={toggleFollow} onLogout={handleLogout} onEditProfile={() => setShowEditProfile(true)} onOpenPost={openPost} onMessage={(u) => { setTab("dms"); openChat(u); }} />
          )}
        </main>
      </div>
