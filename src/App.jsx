...
function BottomNav({ tab, setTab, avatarUrl }) {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[rgb(var(--bg-rgb))] border-t border-[rgb(var(--fg-rgb)/10%)] flex justify-around items-center py-2.5 z-30">
      ...
      <button onClick={() => setTab("profile")} aria-label="Profile">
        <img
          src={avatarUrl}
          alt=""
          className={`w-6 h-6 rounded-full object-cover ${tab === "profile" ? "ring-2 ring-[rgb(var(--fg-rgb))]" : "ring-1 ring-[rgb(var(--fg-rgb)/25%)]"}`}
        />
      </button>
    </nav>
  );
